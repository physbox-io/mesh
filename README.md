...
### CI (GitHub Actions) and the `deploy` branch

`.github/workflows/test.yml` runs on every push to `main` and every pull
request against it: `npm ci`, then typecheck, lint and the Vitest suite. On a
green push to `main` — and only then — the workflow force-pushes that same
commit to the `deploy` branch, which is what Cloud Build watches.

`deploy` is not a branch to work on. It is a pointer at the last commit that
passed, moved only by the workflow, and only ever to a commit already on
`main`. The consequence is that a red `main` cannot reach production, and the
image build no longer has to re-run the typecheck: the Dockerfile uses
`build:image` (`vite build` alone) because `npm run typecheck` already gated
it. Same arrangement as Etch and Volt.

To deploy a commit by hand — a rollback, say — push it to `deploy` yourself:

```bash
git push --force origin <sha>:refs/heads/deploy
```

### Cloud Run deploy (Cloud Build trigger, not checked into this repo)

Pushes to `deploy` deploy automatically via the `physbox-deploy` Cloud Build
