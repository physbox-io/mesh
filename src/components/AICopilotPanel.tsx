  ...
  return (
    <aside className="w-full sm:w-96 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-l border-slate-200 dark:border-slate-800 flex flex-col h-full shrink-0 shadow-2xl z-40 absolute right-0 inset-y-0 sm:relative animate-in slide-in-from-right-8 duration-300">
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50/80 dark:bg-slate-950/20 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 dark:text-slate-550 font-medium">Gemini 3.5 Flash</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden p-4 gap-4">
        ...
        {/* Navigation Modes */}
        <div className="grid grid-cols-3 gap-1 p-1 bg-slate-100 dark:bg-slate-950/40 rounded-xl select-none">
          <button
        ...
        {mode !== 'explain' ? (
          <div className="flex flex-col gap-2 shrink-0">
            <textarea
              ...
              className="w-full px-3 py-2 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs shadow-inner bg-white dark:bg-slate-955 text-slate-800 dark:text-slate-200 min-h-[90px] leading-normal placeholder-slate-400 dark:placeholder-slate-500"
            />
            <button
              onClick={mode === 'generate' ? handleGenerate : handleMutate}
              disabled={loading}
              className="py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-md shadow-blue-150 dark:shadow-none flex items-center justify-center gap-1.5 transition-all cursor-pointer disabled:opacity-50"
            >
        ...
        {!apiKey && (
          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/60 p-3.5 rounded-xl shrink-0 flex flex-col gap-2 shadow-inner">
            <div className="flex gap-2 items-start">
              ...
              <div className="flex flex-col">
                ...
                <p className="text-[10px] text-amber-650 dark:text-amber-450 leading-normal">AI Copilot needs a Gemini API Key to run. Configure it below:</p>
              </div>
            </div>
            <input 
              type="password" 
        ...
        <div ref={responseContainerRef} className="flex-1 border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/40 p-4 rounded-xl overflow-y-auto leading-relaxed text-xs text-slate-700 dark:text-slate-300 shadow-inner min-h-[150px]">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full gap-2 py-8 text-slate-400 dark:text-slate-500 select-none">
